...
import { IAuthApiResponse, ILoginPayload } from "@/interfaces/interfaces";
...
const Login = () => {
  ...
  const [creds, setCreds] = useState<ILoginPayload>({
    email: "",
    password: "",
  });
  ...
  const handleLogin = async (e: FormEvent) => {
    e.preventDefault();
    const res: IAuthApiResponse = await loginUser(creds);
    if (!res.error) {
      setUser(res.user);
      router.replace("/");
    } else {
